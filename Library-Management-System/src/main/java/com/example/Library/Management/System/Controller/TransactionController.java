...
public class TransactionController {
    @Autowired
    private TransactionService tsObj;

    @PostMapping("/issueBook/{bookId}/{cardId}")
    public ResponseEntity issueBookBook(@PathVariable("bookId")Integer bookId,
                                        @PathVariable("cardId")Integer cardId)
    {
        try{
            String result=tsObj.issueBook(bookId, cardId);
    ...
    {
        try{
            String result=tsObj.returnBook(bookId,cardId);
            return new ResponseEntity(result,HttpStatus.OK);
        }
        catch(Exception e)
        {
